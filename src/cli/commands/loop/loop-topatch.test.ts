...
describe('toPatch', () => {
  it('maps critics: name, score, abstain=false, reason=first finding', () => {
    ...
    const r = toPatch(ctx({ critiques }), new LoopMetrics(), LoopState.PLAN);
    ...
    expect(r.critics?.[1]).toMatchObject({
      ...
      score: 0.5,
      abstain: false,
      reason: 'passes without issues',
    });
  });

  it('returns empty critics when ctx.critiques is undefined', () => {
    const r = toPatch(ctx(), new LoopMetrics(), LoopState.PLAN);
    expect(r.critics).toEqual([]);
  });

  it('passes loop state and finished flag', () => {
    expect(toPatch(ctx(), new LoopMetrics(), LoopState.DONE).loop).toBe('DONE');
    expect(toPatch(ctx(), new LoopMetrics(), LoopState.DONE).finished).toBe(true);
    expect(toPatch(ctx(), new LoopMetrics(), LoopState.PLAN).finished).toBe(false);
  });
  ...
  it('maps eval gate from ctx.eval.passed (truthy/falsy)', () => {
    ...
    const r2 = toPatch(ctx(), new LoopMetrics(), LoopState.PLAN);
  ...
  it('maps weightedAvg from ctx.aggregate.score (fallback 0)', () => {
    ...
    expect(toPatch(ctx(), new LoopMetrics(), LoopState.PLAN).eval?.weightedAvg).toBe(0);
  });

  it('maps prCi.ciStatus per loop state', () => {
    expect(
      toPatch(ctx({ prUrl: 'https://x' }), new LoopMetrics(), LoopState.CI_WATCH).prCi,
    ).toMatchObject({ prUrl: 'https://x', ciStatus: 'pending' });
    expect(
      toPatch(ctx({ prUrl: 'https://y' }), new LoopMetrics(), LoopState.DONE).prCi,
    ).toMatchObject({ prUrl: 'https://y', ciStatus: 'green' });
    expect(toPatch(ctx(), new LoopMetrics(), LoopState.PLAN).prCi).toMatchObject({
      prUrl: undefined,
      ciStatus: undefined,
    });
  });
  ...
  it('passes recoverAttempts from metrics and aborted flag', () => {
    ...
    m.recoverAttempts = 3;
    const r = toPatch(ctx({ aborted: true }), m, LoopState.RECOVER);
