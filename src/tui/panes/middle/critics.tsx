...
const WEIGHTS: Record<string, number> = {
  security: 1.5, perf: 1.0, architecture: 1.5, testing: 1.0, doc: 1.0,
  devops: 1.0, legal: 1.0, privacy: 1.5, style: 1.0, dx: 0.8,
  accessibility: 1.0, maintainability: 1.0, sloc: 1.0, imports: 1.5, todo: 1.0,
};

const ICONS: Record<string, string> = {
  security: '🛡', perf: '⚡', architecture: '🏛', testing: '✓', doc: '📖',
  devops: '⚙', legal: '⚖', privacy: '🔒', style: '🎨', dx: '✦',
  accessibility: '♿', maintainability: '🔧', sloc: '⊟', imports: '⇄', todo: '✗',
};
...
export function Critics({ critics, weightedAvg, threshold }: CriticsProps) {
  ...
  return (
    <Box flexDirection="column" borderStyle="round" borderColor={colors.scoring} paddingX={1} flexGrow={1}>
      <Text color={colors.scoring} bold>◉  CRITICS ({critics.length}/{names.length} reported)</Text>
      {names.map((name) => {
        const c = known[name];
        ...
        if (!c) {
          return (
            <Box key={name}>
              <Text color={colors.fgDim}>{icon} {name.padEnd(16)}  weight={w.toFixed(1)}  —  —    abstain</Text>
            </Box>
          );
        }
        if (c.abstain) {
        ...
        const color = c.score >= threshold ? colors.done
          : c.score < 0.4 ? colors.failed
          : colors.warn;
        return (
          <Box key={name}>
            <Text color={color}>{icon} {name.padEnd(16)}  w={w.toFixed(1)}  </Text>
      ...
      <Box marginTop={1}>
        ...
        <Text color={passed ? colors.done : colors.warn} bold>{formatScore(weightedAvg)}</Text>
        <Text color={colors.fgDim}>/ {threshold.toFixed(2)}</Text>
        <Text color={passed ? colors.done : colors.warn} bold>  {passed ? '→ PASS' : '→ FAIL'}</Text>
      </Box>
    </Box>
  );
}
