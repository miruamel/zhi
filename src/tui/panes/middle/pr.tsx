...
function ciColor(status: PrCiState['ciStatus']): string {
  switch (status) {
    case 'green': return colors.done;
    case 'red': return colors.failed;
    case 'pending': return colors.running;
    case 'unknown':
...
function ciLabel(status: PrCiState['ciStatus']): string {
  switch (status) {
    case 'green': return '✓ CI green';
    case 'red': return '✗ CI red';
...
export function Pr({ prCi }: PrProps) {
  return (
    <Box flexDirection="column" borderStyle="round" borderColor={colors.commit} paddingX={1} flexGrow={1}>
      <Text color={colors.commit} bold>⇡  PR / CI</Text>
      {prCi.prUrl ? (
        <Box gap={1}>
