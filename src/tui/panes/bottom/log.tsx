...
function kindColor(k: LogEntry['kind']): string {
  switch (k) {
    case 'transition': return colors.accent;
    case 'gate': return colors.warn;
...
export function Log({ log, expanded, maxLines }: LogProps) {
  ...
  return (
    <Box flexDirection="column" borderStyle="round" borderColor={colors.fgDim} paddingX={1} flexGrow={1}>
      <Text color={colors.fgDim} bold>ℹ  LOG ({log.length} entries{hiddenCount > 0 ? `, ${hiddenCount} hidden` : ''})</Text>
      {visible.length === 0 ? (
        ...
        visible.map((e, i) => (
          <Box key={i} gap={1}>
            ...
            <Text color={kindColor(e.kind)} bold>{e.kind.padEnd(10)}</Text>
