...
Most agent tools today (Claude Code, OMP, Aider, KiloCode, Hermes) are chat wrappers with tool calls. They're great for interactive work, but they don't _finish_ the job. Zhi takes a different angle: a state machine `INTAKE → PLAN → ISOLATE → EXECUTE → CRITIQUE → EVALUATE → COMMIT → PR_OPEN → CI_WATCH → DONE`, with every transition guarded by a machine-decidable gate.
