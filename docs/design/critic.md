...
Additional critics (Doc, DevOps, Legal, Privacy, DX, Accessibility, Security, Perf, Testing, Style) were promoted from stub → concrete gradually in `docs/guides/roadmap.md` v0.2.0+. Each addition = new `plant/<name>/` directory + register in `composeCritiques` + test. Per-critic semantics (what is measured, weights, penalty) deserve a short ADR.
