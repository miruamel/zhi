...
/** @brief Mock runner yang mengembalikan laporan bersih (tanpa pelanggaran). @since 0.1.8 */
function cleanCruiserRunner(): CruiserReport {
  return {
    modules: [{ source: 'test.ts', dependencies: [], orphan: false, valid: true }],
  };
}

test('compose runs all eleven critics', () => {
  const cr = composeCritiques(
    [{ path: 'engine/foo/a.ts', content: 'export const x = 1;\n' }],
    cleanCruiserRunner,
  );
