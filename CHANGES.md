...
- **P0 npm token leak in git history — redacted and scrubbed** — npm publish token `***REDACTED***` was embedded in plaintext at `audit-log/entries/2026-09-04-state-sync-2.md` line 47 (commit `9cbec75`). The token appeared in exactly one file across one commit — no other file or commit was affected. Remediation: (1) confirmed token already invalid (E401 on `npm token ls`) — consistent with prior rotation tracked in §7.1; (2) targeted redaction via `git filter-repo --replace-text` (526 commits rewritten, 0 occurrences remaining); (3) backup tag `backup-before-token-scrub` at `ae08b71` retained for audit trail; (4) PR #60 (full-history rewrite, 100+ files) closed — disproportionate and unnecessary given single-file exposure. Working tree verified clean: `grep -rn` across all tracked files returns 0 matches. Tracked in `audit-log/entries/2026-09-04-npm-token-leak-incident.md` and issue #35.
