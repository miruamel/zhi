...
- **Roadmap critic status correction** — `docs/guides/roadmap.md` claimed `critic/*` had "8 stubs"; all 15 critics are concrete implementations (accessibility, architecture, doc, devops, dx, legal, testing, imports, maintainability, perf, privacy, security, sloc, style, todo). Updated v0.1.0 scope and struck through the v0.2.0 "remaining 8 critics" bullet with a DONE marker.
...
- **README stale facts corrected** — test count 229→255, repo visibility status. (`df07446`)
- **Closed PR #47 (`fix/tui-tsc-debt`) as superseded** — the tsc debt it targeted is already resolved on `main` (typecheck clean). The PR branch carries a parallel TUI lineage (widgets, panes, integration subdirs) that diverged before PR #45 was merged and duplicates main's. Merge state was DIRTY/CONFLICTING; rebase fails on first commit. CI only ran `architecture-guard` — the `ci` workflow (which includes `typecheck`) never executed on this branch, leaving 14+ type errors (ink-testing-library missing, Ink `onClick` prop violations, render callback signature mismatches). Per Mandate §5.3 ("Tidak merge jika blocking tersisa atau CI gagal") and §2.3 (max 5 open PRs), closed without merge. Branch `fix/tui-tsc-debt` deleted locally + remotely.
- **Deleted stale `fix/tui-tsc-debt` branch** — PR #47 closed as superseded; branch diverged 38 commits ahead of merge base, 70 behind main. Deleted local + remote after closure.
- **Closed PR #48 (`fix/tui-tsc-debt-2-widgets`) as superseded** — same pattern as PR #47: a parallel TUI lineage descending from PR #46's `feat/tui-ink` (closed as superseded). The PR targets `src/tui/widgets/data/json-view/renderers.tsx`, `src/tui/widgets/data/table.tsx`, and `src/tui/widgets/wayfinding/breadcrumb.tsx` — **none of which exist on main**. Main's TUI lives in `src/tui/panes/` + `src/tui/core/` (PR #45 lineage); there is no `widgets/` subdirectory. The PR's 5 widget Ink prop fixes have no corresponding code on main to apply to. Merge state was DIRTY; 39 commits since merge base; CI only ran `architecture-guard` (the `ci` workflow with `typecheck` never executed). Per Mandate §5.3 and §2.3, closed without merge. Remote branch `fix/tui-tsc-debt-2-widgets` deleted.
- **Closed PR #49 (`fix/tui-tsc-debt-3-tests`) as superseded** — fourth in the divergent TUI lineage (PRs #46/#47/#48/#49). Targets `src/tui/widgets/`, `src/tui/panes/top/network/`, `src/tui/panes/middle/queue/` — **none of which exist on main**. Main's TUI lives in `src/tui/panes/` + `src/tui/core/` (PR #45 lineage); there is no `widgets/` subdirectory. Merge state was DIRTY; 40 commits, 184 files changed. CI only ran `invariants` + "Kilo Code Review" — the `ci` workflow (which includes `typecheck`) never executed on this branch. Main typecheck is clean (0 errors). Per Mandate §5.3 and §2.3, closed without merge. Remote branch `fix/tui-tsc-debt-3-tests` deleted.
- **Closed PR #50 (`fix/tui-tsc-debt-4-app`) as superseded** — fifth in the divergent TUI lineage (PRs #46/#47/#48/#49/#50). Targets `src/tui/{app,render,state,colors,format,icons,keymap,viewer,viewer.test}.tsx` and `src/tui/panes/{header,dag,detail,critics,eval,pr,log,help}.tsx` — **none of which exist on main**. Main's TUI lives in `src/tui/panes/` + `src/tui/core/` (PR #45 lineage). Merge state was DIRTY; 184 files changed, 13883 insertions. CI only ran `architecture-guard` — the `ci` workflow (with `typecheck`) never executed on this branch. Main typecheck is clean (0 errors). Per Mandate §5.3 and §2.3, closed without merge. Remote branch `fix/tui-tsc-debt-4-app` deleted.
- **`index.test.ts` engine import depth corrected** — `cface9e` fixed a `../../../../engine/...` deep-relative import in the stream test after the co-location refactor. Superseded by the `@engine/*` path-alias migration.
