...
- **`publish.yml` stage 6: dedupe asset globs in `gh release create` (Issue #140, PR #141)** — Run `33986869242` — the stage 5 fix worked (`GH_TOKEN: ***` appears in the step env dump) and `gh release create` progressed all the way to asset upload, then failed with `HTTP 422: ReleaseAsset.name already exists` on `zhi-linux-x64.sha256`. Root cause: three globs overlapped — `artifacts/zhi-*` already matches all 8 files (4 binaries + 4 SHA256 checksums), so `artifacts/*.sha256` re-uploaded each checksum and `gh` rejected the duplicate; `artifacts/*.exe` was redundant too (windows-x64 is the only `.exe` and `zhi-*` covers it). Fixed with a single glob `artifacts/zhi-*`. Verified: the partial release object `383349836` returns 404 afterward — `gh release create` rolls back tag + object on any asset failure, so no residue to clean up; the remote tag `v0.1.6` (`436352e`) is untouched. Stages 5 and 6 were the last two blockers before a complete `v0.1.6` GitHub Release (tag + 4 platform binaries + 4 SHA256 checksums + release notes).
