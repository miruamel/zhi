...
- **CI workflow changes skipped typecheck and tests (#309)** — `scripts/gate.ts` now classifies `.github/workflows/*` as non-docs, so `--if-changed` runs the full gate for CI changes. Added a focused regression test and guarded the gate entry point for test imports.
