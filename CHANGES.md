...
- **Git hooks re-installed with token-scanning pre-commit hook** — `.git/hooks/pre-commit` now runs token scan (blocks `npm_[A-Za-z0-9]{20,}` patterns in staged diff) BEFORE `bun x lint-staged`. `.git/hooks/commit-msg` runs `bun x commitlint --edit "$1"`. Hooks lost during `git-filter-repo` history scrub; re-established and verified: bad commit rejected (commitlint EXIT=1), good commit accepted (EXIT=0), token pattern blocked (EXIT=1 before lint-staged runs). Hooks are git-native, not committed to git.

### CI

- **Prettierignore audit-log exclusion cherry-picked to main (PR #61)** — 3 commits from stale `fix/prettierignore-audit-log-entries` applied: `.prettierignore` now excludes `audit-log/entries/` and `audit-log/README.md`. Stale branch deleted (local + remote); `alert-autofix-1` remote deleted. README entry count reconciled: 71 files on disk = 71 listed.
